...
这篇博文旨在完整记录整个技术选型和落地过程。我不只是写技术路线，而是要把踩过的坑和关键的操作命令记下来，方便日后重装或迁移时查阅。

主要内容分为三部分：

1. **基础设施**：搞定内网穿透，实现远程访问。
2. **静态网站**：利用 Hexo、GitHub 和 Cloudflare 搭建个人博客。
3. **动态应用**：从零开发并容器化部署一个前后端分离的云便签应用。

------

## 第一章 基础设施与远程访问

这是地基部分。核心诉求很简单：我人在外面（比如公司），要能访问家里服务器上的网页，还要能通过终端控制它。

### 1.1 公网 IP 路线（已放弃）

最开始的想法是直接申请公网 IP。

- **IPv4**：给移动运营商打电话，申请公网IPv4，理由是装监控。对方答复现在资源枯竭不给家庭用户办理。家庭宽带是类似 `10.x.x.x` 的大内网 IP（CGNAT），没有公网 IPv4 访客就进不来。此路不通。
- **IPv6**：现在的光猫和路由器基本都支持 IPv6。我尝试开启了 IPv6，确实能获取到全球唯一的地址。但是测试后发现致命问题：
  1. **防火墙限制**：家里的路由器无法关闭 IPv6 防火墙，这意味着外部流量进不来。
  2. **网络环境受限**：很多办公场所或公共 Wi-Fi 只支持 IPv4，根本连不上家里的 IPv6 地址。
  3. **端口封锁**：运营商对家庭宽带封锁了 80 和 443 端口。这意味着我以后部署网站，必须要在域名后面带个丑陋的端口号（比如 `:8080`），而且没法自动申请 SSL 证书。
...
我在注册商买了 `cclog.top`，费用很低。买好后的第一件事，就是修改域名的 DNS 服务器（Nameservers），把它托管给 Cloudflare。
...
1. **设置公网映射** (Public Hostname)：
   ...
   - Subdomain: `ssh` (即 `ssh.cclog.top`)
   - Service: `ssh://localhost:22`
   - 这一步告诉 Cloudflare，把访问 `ssh.cclog.top` 的流量，转发给我服务器的 22 端口。

2. **配置应用策略** (Access Application)：

   光映射不行，还得有界面。

   - 在 Zero Trust 左侧菜单 `Access` -> `Applications` -> `Add an application` ，选择 `Self-hosted`。
   - Application name: 填个名字比如 "SSH Web"。
   - Session Duration: 设置登录一次管多久，比如 "24 hours"。
   - Domain: 选择刚才配置的 `ssh.cclog.top`。
   - 添加身份验证规则
     - `Rule name`: `Allow Me`
     - `Rule action`: `Allow`
     - `Selector`: `Email`
     - `Value`: 邮箱地址
   - 开启浏览器渲染: `Settings` 页面找到 `Browser rendering`，选择 `SSH`。

**效果**：

保存后，我在任何浏览器的地址栏输入 `ssh.cclog.top`，就会弹出一个 Cloudflare 的身份验证页（验证邮箱验证码），验证通过后直接在浏览器里渲染出一个终端窗口，输入服务器账号密码即可登录。
...
再次强调：我们要传到 GitHub 上的，不是 `public` 文件夹，而是除了 `public` 和 `node_modules` 之外的所有源文件。
...
1. 在 GitHub 上新建一个私有仓库，命名为 `cc-blog-source`。私有是为了保护草稿和配置隐私。
2. 在本地博客根目录初始化 Git：
...
2. 授权访问我的 GitHub 账号，并选择刚才创建的 `cc-blog-source` 仓库。
...
整个过程大概耗时 1-2 分钟。以后我写完文章，只需要在本地执行：
...
tips: 当时我进行到这一步还遇到一点麻烦，就是 Cloudflare 虽然提供免费 `application` 服务，但是要求你绑定支付方式。我没有 Visa 或 MasterCard，在这一步还困了一些时候。最后我找到的解决办法是 **PayPal** ，它被 Cloufflare 接受且可以绑定国内的银联卡。

为什么一开始没有想到 PayPal 呢？因为 Cloudflare 的页面设计有一点坑：在具体的某项目服务下绑定支付方式，可选项里没有 PayPal; 但当你退回到主页面的个人资料里去绑定的时候，就能看见 PayPal 了。

### 2.5 域名绑定

最后，给博客一个体面的入口。

在 Cloudflare Pages 的项目设置里，点击 `Custom domains`，输入 `blog.cclog.top`（后改变主意，直接用主域名 `cclog.top`）。
...
静态博客是对外展示，动态应用则是对内服务。这一部分涵盖了全栈开发、容器化部署、网络隔离的完整过程，是技术含量最高、逻辑最复杂的部分。

这一章的目标是：从零开发一个前后端分离的私有便签应用（cc Notes），并将其容器化部署到服务器上。它必须满足三个核心需求：
...
作为一个轻量级的个人工具，我没有选择厚重的 Java/Spring 或复杂的 K8s，而是选择了短小精悍的 Python 云原生栈：
...
为了保持项目整洁，我采用了单文件架构 (`main.py`)。核心逻辑包括：

1. **数据模型**：定义了前端传来的数据格式（`content`）和返回的数据格式（`id`, `timestamp`）。

2. **中间件鉴权**：编写了一个依赖注入函数 `verify_token`。它会拦截每一个请求，检查 HTTP Header 中的 `X-cc-Token`。
...
EXPECTED_TOKEN = os.getenv("SECRET_TOKEN", "cc_dev_token")

# 依赖注入：门神验证
async def verify_token(x_token: str = Header(..., alias="X-cc-Token")):
...
在调试时，发现即便输入了错误的 Token，前端有时也会错误地进入工作页面。
...
至此，我的私有云便签应用 "cc Notes" 已经稳稳地运行在旧服务器上了。

回顾这部分工作，最大的收获不在于代码本身，而在于对 **网络接口绑定**、**Docker 网络隔离** 以及 **数据持久化** 的理解。这些在本地开发环境很难遇到的问题，在服务器部署实战中给我上了宝贵的一课。
